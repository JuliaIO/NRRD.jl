...
function FileIO.load(f::File{format"NRRD"}; mmap=:auto)
    st = open(f)
    skipmagic(st)
    nrrd = load(st, mmap=mmap)
    close(st)
    nrrd
end
...
function FileIO.save(f::File{format"NRRD"}, img::Image; props::Dict = Dict{ASCIIString,Any}())
